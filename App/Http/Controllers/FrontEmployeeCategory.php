...
class FrontEmployeeCategory extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $post = EmployeeCategory::whereId($request->hidden_id)->first();
        if ($post) {
            $post->name = $request->name;
            $post->update();
            if ($post) {
    ...
    {
        if (request()->ajax()) {
            $data = EmployeeCategory::whereId($id)->first();
    ...
    {
        $data = EmployeeCategory::with('employees')->findOrFail($id);
        if ($data) {
